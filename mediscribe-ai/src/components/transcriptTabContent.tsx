...
export const TranscriptTabContent = () => {
  ...
  const handleSummary = async () => {
    if (!transcript) {
      toast.error(t("summary:alert.error.transcriptNotFound.title"), {
        description: t("summary:alert.error.transcriptNotFound.description"),
      });
      return;
    }
    ...
    try {
      ...
      toast(t("summary:alert.success"));
      setActiveTab("summary");
    } catch (error) {
      console.error(error);
      toast.error(t("summary:alert.error.summaryError.title"), {
        description: t("summary:alert.error.summaryError.description"),
      });
    }
  };
    ...
    <TabsContent value="transcript" className="focus-visible:outline-none">
      <div className="space-y-8 py-8">
        <div className="flex justify-center mb-4">
          <FeatureIcon icon={<Icons.fileText className={ICON_SIZE_LG} />} className="w-20 h-20" />
        </div>
        ...
        <div className="flex justify-start mt-6">
          ...
          >
            <Icons.clipboardPaste className={ICON_SIZE_SM} /> {t("transcript:example")}
          </Button>
        </div>
        ...
        <div className="flex justify-center mt-6">
          <Button
            ...
            className={`cursor-pointer gap-2 px-8 py-6 text-lg rounded-full bg-gradient-to-r ${GRADIENT_PRIMARY} hover:shadow-lg hover:shadow-blue-500/30 transition-all text-white`}
          >
            <Icons.clipboardList className={ICON_SIZE_SM} /> {t("summary:generate")}
          </Button>
        </div>
      </div>
    </TabsContent>
